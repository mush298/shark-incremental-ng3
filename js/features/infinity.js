let INFINITY = {
    ...
    power_formula(x) {
        ...
        return E(1).add(x.max(10).log10().pow(0.75)).pow(hasInfinityUpgrade(10) ? 1.25 : 1).mul(hasCE(1) ? CORRUPTION.effects[1].effect(0) : 1)
    }
}
...
function setupInfinityHTML() {
    ...
    for (let i = 0; i < CORRUPTION.dims; i++) {
        k += `  
    <button class = "corrupted-dimension" id = "cd${i}">
