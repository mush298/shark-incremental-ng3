...
function setupTopCurrenciesHTML() {
    ...
    for (let [i,x] of Object.entries(TOP_CURR)) {
        ...
        <div class="curr-top" id="curr-top-${i}-div">
            <div id="curr-top-${i}-amt1"><span id="curr-top-${i}-amt2">???</span> ${CURRENCIES[x.curr].costName} <div id="curr-top-${i}-amt3"><span id="curr-top-${i}-amt4">???</span> ${toTextStyle('Infinities', 'infinity')}</div></div><button onclick="doReset('${x.reset ?? x.curr}')" id="curr-top-${i}-btn">Reset</button>
        </div>
        `
        } else {
            h += `
